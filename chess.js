...
var GameController = Object.subClass({
  ...
  boardInit: function() {
    ...
    this.gCanvasElement.addEventListener("click", this.classOnClick, false);
  ...
  fillBack: function(cell, style) {
    if(style != null) {
      ...
      this.gDrawingContext.fillRect(cell.column * this.kPieceWidth + 1, cell.row * this.kPixelHeight + 1,
  ...
  drawBArea: function(piece, team, style) {
    for(var i=1; (parseInt(piece.column) + i) < this.kBoardWidth && (parseInt(piece.row) + i) < this.kBoardHeight; i++) {
      if(this.pieceJudge(parseInt(piece.column) + i, parseInt(piece.row) + i, parseInt(piece.row) + i, team, style)) {
        break;
      }
    }
        ...
        if(i==0 && j==0) {
          ...
          this.kBoardHeight > (piece.row + j) && (piece.row + j) >= 0) {
            if(this.gCells[piece.column + i][piece.row + j].BAtack == 0 && team == 'W') {
              ...
              this.pieceJudge(parseInt(piece.column) + i, parseInt(piece.row) + j, tram, style);
  ...
  movePiece:function(col, row) {
    ...
    var selectedPieceTemp = this.gSelectedPiece;
    ...
    if(targetCell === undefined || targetCell === null) {
      ...
      if(this.checkMateSimu(selectPieceTemp, col, row)) {
        //自分自身のチームのチェック
        alert("checkMate");
      } else {
        ...
        selectPieceTemp.row = row;
        selectPieceTemp.column = col;
        this.turnChange();
        this.viewRefresh();
